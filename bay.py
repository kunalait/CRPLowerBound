...
class Bay:
    ...
    def __str__(self):
        ...
        for t in reversed(range(h_max)):
            ...
            builder.append(' | ')
            builder.append(''.join('[{}]'.format(str(pri[s][t]).rjust(cell_width))
                                   if t < h[s]
                                   else '[{}]'.format(' ' * cell_width)
                                   for s in range(n_stacks)))
            builder.append('\n')
    ...
    def brp_min_max(self):
        ...
        while n_blocks > 0:
            ...
            (s_target, t_target) = next((s, t) for s in range(n_stacks) for t in reversed(range(h[s]))
                                        if pri[s][t] == min_value and t + 1 >= n_blocks - (n_stacks - 1) * n_tiers)
            while t_target < h[s_target] - 1:
                p = pri[s_target][h[s_target] - 1]

                s_dest = min((s for s in range(n_stacks) if s != s_target and h[s] < n_tiers and p <= qlt[s][h[s] - 1]),
                             key=lambda s: qlt[s][h[s] - 1],
                             default=max((s for s in range(n_stacks) if s != s_target and h[s] < n_tiers),
                                         key=lambda s: qlt[s][h[s] - 1]))
